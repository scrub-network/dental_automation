...
class DentalJobCafe:
    ...
    def scrape_each_page(self):
        ...
        batch_df = pd.DataFrame(columns=["job_id", "location", "state", "date_posted", "employer", "employer_type", "job_type", "days_of_week", "description", "post_link", "source", "created_at"])
        ...
        for url in self.website_links:
            ...
            employer_type = generate_dso_mapping(company, self.dso_mapping)

            state = get_full_state_name(location)

            # Append to dataframe
            new_row = {"job_id": job_id, "location": location, "state": state, "date_posted": date_posted, "employer": company, "employer_type": employer_type,
    ...
    def execute(self):
        ...
        while int(max_page) >= page_num:
            ...
            # Write to database
            batch_df.to_sql("source_dentist_job_cafe", con=self.engine, if_exists="append", index=False)

            all_jobs = pd.concat([all_jobs, batch_df])
            page_num += 1

if __name__ == "__main__":
    scraper = DentalJobCafe()
    scraper.execute()